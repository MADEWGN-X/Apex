...
# // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Dev > Mas3ko${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green}Mas3ko ® ${NC}${YELLOW}(${NC} ${green} 3KO⭐ ${NC}${YELLOW})${NC}"
echo -e " © Recode By My Self MAS3KO ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC
...
restart_system() {
    ...
    echo -e "\e[32mloading...\e[0m"
    clear
    izinsc="https://ip.madewgn.my.id/ip"
    # USERNAME
    rm -f /usr/bin/user
...
function install_xray() {
    ...
    # // Ambil Config Server
    wget -O /etc/xray/trojan.json "${REPO}config/trojan.json" >/dev/null 2>&1
    wget -O /etc/xray/vmess.json "${REPO}config/vmess.json" >/dev/null 2>&1  
    wget -O /etc/xray/vless.json "${REPO}config/vless.json" >/dev/null 2>&1  
